...
function montaTabelaDePrecos(qtdPaes, valorDoPao) {
    console.log(`Preço do pão: R$${valorDoPao}.
Panificadora Pão de Ontem -  Tabela de preços: \n`);

let linhasDaTable = 1;
while(linhasDaTable <= qtdPaes) {
    console.log(`${linhasDaTable} - R$${(linhasDaTable * valorDoPao).toFixed(2)}`)
    linhasDaTable++
}
