...
function montaTabelaDePrecos(numeroDeProdutos, valorDosProdutos) {
    let linhasDasTable = 0;

    console.log(" Lojas Quase Dois - Tabela de preços")
    while(linhasDasTable < numeroDeProdutos){
        console.log(`${linhasDasTable+1} - R$${((linhasDasTable+1) * valorDosProdutos).toFixed(2)}`)
        linhasDasTable++;
    }
}

montaTabelaDePrecos(50, 1.99)